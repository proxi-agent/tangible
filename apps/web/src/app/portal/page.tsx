  ...
  return (
    <>
      ...
      {report.isLoading ? (
        ...
        <Report report={report.data} sites={sites.data ?? []} />
      ) : null}
    </>
  );
}

function Report({ report, sites }: { report: SavingsReport; sites: EngagementSite[] }) {
      ...
      {priced.length > 0 ? (
        <Card>
          <CardHeader
            ...
            description={
              report.assessed
                ? 'From what the district has you at today to what a corrected return supports. Click any line to see the assets behind it.'
                : 'From what your register says today to what a corrected return supports. Click any line to see the assets behind it.'
            }
