...
import { day, dayShort, moneyExact, plural } from '@/lib/format';
...
function Recorded({ notice, engagementId }: { notice: AssessmentNotice; engagementId: string }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {notice.correction ? <StillOpen outlook={notice.correction} /> : null}
    </div>
  );
}
 ...
 */
function StillOpen({ outlook }: { outlook: CorrectionOutlook }) {
  ...
  return (
    <div className="space-y-2 rounded-md border border-[var(--color-hairline)] bg-[var(--color-surface)] p-2.5">
      ...
      >
        ...
        <span className="font-medium">After the protest window</span>
        <ChevronDown
          className={cn(
