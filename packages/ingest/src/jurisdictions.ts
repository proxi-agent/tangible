import { bool, lit, num, type Warehouse } from '@tangible/analytics';
...
interface JurisdictionRow {
  ...
  years: unknown;
  publishes_filing_status: unknown;
}
...
export async function listJurisdictionSummaries(
  ...
  const rows = await warehouse.query<JurisdictionRow>(/* sql */ `
    SELECT
      ...
      coalesce(stats.publishes_filing_status, FALSE) AS publishes_filing_status,
    ...
    LEFT JOIN (
      SELECT
        ...
        -- Any non-null filing status at all. A county whose connector maps the
        -- column but whose file leaves it blank counts as not publishing it.
        count(*) FILTER (WHERE rendition_filed IS NOT NULL) > 0
                                        AS publishes_filing_status,
  ...
  const loaded = rows.map((row): JurisdictionSummary => ({
    ...
    publishesFilingStatus: bool(row.publishes_filing_status),
  ...
  const available = listJurisdictions()
    ...
    .map((j): JurisdictionSummary => ({
      ...j,
      accountCount: 0,
      // Unknown until something is loaded, and false is the safe reading: a
      // view that would open on a filing filter opens on value instead.
      publishesFilingStatus: false,
      latestYear: null,
      lastIngestedAt: null,
    }));
