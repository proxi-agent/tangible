...
Tangible models the election per site. When every asset at a site is valued on the district's published schedules, none is still under review, and the total is at or under the exemption for the tax year, the draft takes the election automatically: it answers the Section 5 market-value question at $125,000 or less, ticks the certification box that 22.24(c) requires the form to carry, and leaves every schedule blank on the district's copy. The rendition screen and the returns board say "certification" rather than "rendition" for such a site, the record freezes that it was filed as one, and the file copy keeps the unfiled schedules so next season has a figure to compare to. A preparer can untick the election to render in full anyway, and asking for it where the value cannot be known is refused with a blocker rather than sworn to.

Two parts of the test are not in the register and are asked of the signer instead, as a warning on every certifying return: whether a related entity holds property at the same location, since 11.145(f) aggregates a unified business enterprise, and whether the property is leased out or kept at premises the owner neither owns nor leases, since 11.145(d) and (d-1) then allow one exemption per taxing unit regardless of location. The election is also Texas-only and begins with tax year 2026; a Florida return, or a Texas return for an earlier year, is always a full rendition. What Tangible does not yet carry is the election's standing effect: 22.01(j-3) makes it run from the following tax year until ownership changes, so a site certified last season owes nothing this season unless its value has crossed the line or the chief appraiser has required a rendition, and the returns board will nonetheless draft it another certifying return. Filing that is permitted, not required.`,
